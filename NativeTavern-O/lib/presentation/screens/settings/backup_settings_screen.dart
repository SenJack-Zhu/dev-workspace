...
import 'package:native_tavern/domain/services/sqlite_backup_service.dart';
      ...
      body: isLoading
          ...
          : ListView(
              ...
              children: [
                ...
                // SQLite Full Backup with media files (the robust full backup)
                _buildSection(
                  context: context,
                  title: 'SQLite Full Backup (with images & media)',
                  children: [
                    const ListTile(
                      leading: Icon(Icons.folder_zip, color: AppTheme.accentColor),
                      title: Text('Complete backup including images, avatars, backgrounds'),
                      subtitle: Text('Generates a single .ntbackup (zip) file. Restore to any device with all media preserved.'),
                    ),
                    const SizedBox(height: 8),
                    Padding(
                        ...
                        children: [
                              ...
                              onPressed: () async {
                                try {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(content: Text('Creating backup with media...')),
                                  );
                                  final file = await SqliteBackupService.instance.createBackup();
                                  if (!context.mounted) return;
                                  final size = SqliteBackupService.instance.formatFileSize(file.lengthSync());
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(content: Text('Backup ready: ${p.basename(file.path)} ($size)')),
                                  );
                                } catch (e) {
                                  if (context.mounted) {
                                    ScaffoldMessenger.of(context).showSnackBar(
                          ...
                          Expanded(
                            child: FilledButton.icon(
                              ...
                              label: const Text('Backup + Export to /sdcard/Download'),
                              onPressed: () async {
                                try {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(content: Text('Creating backup with media...')),
                                  );
                                  final tempBackup = await SqliteBackupService.instance.createBackup();

                                  // Find a user-writable download directory
                                  // Prefer standard external download paths; fallback to /sdcard/Download
                                  final candidates = [
                                  ...
                                  for (final c in candidates) {
                                    ...
                                    try {
                                      ...
                                      // Test write
                                      final probe = File(p.join(c, '.probe_${DateTime.now().millisecondsSinceEpoch}'));
                                      await probe.writeAsString([1, 2, 3].toString());
                                      await probe.delete();
                                      targetDir = c;
                                      break;
                                    } catch (_) {
                                  ...
                                  if (targetDir == null) {
                                    ...
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      const SnackBar(
                                        content: Text('Cannot write to /sdcard/Download. Sharing via system instead.'),
                                        duration: Duration(seconds: 2),
                                      ),
                                    );
                                    // Fallback: share the backup through system share sheet
                                    await Share.shareXFiles(
                                      [XFile(tempBackup.path)],
                                      text: 'NativeTavern backup (includes images/media) — ${DateFormat('yyyy-MM-dd HH:mm').format(DateTime.now())}',
                                    );
                                    return;
                                  }

                                  final destPath = p.join(targetDir, p.basename(tempBackup.path));
                                  await tempBackup.copy(destPath);
                                  final size = SqliteBackupService.instance.formatFileSize(await File(destPath).length());
                                  if (!context.mounted) return;
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text('Saved to $destPath ($size)'),
                    ...
                    Padding(
                      ...
                      child: FilledButton.icon(
                        ...
                        label: const Text('Restore SQLite Backup (.ntbackup) from file...'),
                        onPressed: () async {
                          try {
                            final l10n = AppLocalizations.of(context);
                            final confirmed = await showDialog<bool>(
                              ...
                              builder: (ctx) => AlertDialog(
                                title: const Text('Restore from .ntbackup'),
                                content: const Text(
                                  'Choose the backup file to restore.\n\n'
                                  'WARNING: Replace mode OVERWRITES your current database AND all media files.\n'
                                  'A pre-restore snapshot of the DB file will be kept alongside the current DB.',
                                ),
                                actions: [
                                  TextButton(
                                  ...
                                  FilledButton(
                                    ...
                                    child: const Text('Pick file & Replace'),
                                  ),
                                ],
                              ),
                            );
                            ...
                            final picked = await FilePicker.platform.pickFiles(
                              dialogTitle: 'Pick .ntbackup file',
                              type: FileType.any,
                            );
                            ...
                            if (!context.mounted) return;
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(content: Text('Restoring ${p.basename(path)}...')),
                            );
                            await SqliteBackupService.instance.restoreBackup(
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                content: Text('Restore done! Restart the app for changes to fully apply.'),
