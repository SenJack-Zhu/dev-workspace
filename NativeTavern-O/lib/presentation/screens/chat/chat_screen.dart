...
class _ChatScreenState extends ConsumerState<ChatScreen> {
  ...
  Future<List<ChatAttachment>> _saveGeneratedImages(
    ...
    final imagesDir = Directory(p.join(dataPath, 'chat_images'));
    ...
    for (final imageBytes in result.images) {
      ...
      final relativePath = await PathUtils.toRelativePath(filePath);
      attachments.add(
        ChatAttachment(
          id: imageId,
          path: relativePath,
    ...
    if (result != null && result.images.isNotEmpty && mounted) {
      ...
      try {
        ...
        for (int i = 0; i < result.images.length; i++) {
          ...
          // Create attachment (store relative path for portability)
          final relativePath = await PathUtils.toRelativePath(filePath);
              ...
              children: [
                ClipRRect(
                  ...
                  child: FutureBuilder<String>(
                    future: PathUtils.toAbsolutePath(attachment.path),
  ...
  Future<void> _addAttachmentFromPath(String filePath) async {
    try {
      // Copy file to app's data directory for persistence (use PathUtils for portability)
      final dataPath = await PathUtils.getDataPath();
      final attachmentsDir = Directory(
        p.join(dataPath, 'attachments'),
      );
...
class _MessageBubbleState extends State<_MessageBubble> {
        ...
        child: GestureDetector(
          ...
          child: ClipRRect(
            ...
            child: ConstrainedBox(
              ...
              child: FutureBuilder<String>(
                future: PathUtils.toAbsolutePath(attachments[0].path),
                builder: (context, snapshot) {
                  if (!snapshot.hasData) {
                    return Container(
                      width: 150,
  ...
  /// Show full-screen image preview with save to gallery option
            ...
            Positioned(
              ...
              child: IconButton(
                ...
                onPressed: () async {
                  ...
                  try {
                    final resolvedPath =
                        await PathUtils.toAbsolutePath(attachment.path);
                        ...
                        if (!granted) {
                          if (context.mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                  content: Text(
                                      l10n.failedToSaveImage('Permission denied'))),
                            );
                          }
                          return;
                        }
                      }
                      await Gal.putImage(resolvedPath, album: 'NativeTavern');
                      if (context.mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                    ...
                    if (context.mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                            content: Text(l10n.failedToSaveImage('$e'))),
                      );
                    }
                  }
                },
