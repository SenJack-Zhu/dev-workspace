...
object ChapterChunker {
    ...
    fun chunkWithOffsets(text: String): List<ChapterChunk> {
        // ── Module hook: chunk.split ──
        // JS modules return JSON: [{"text":"...","start":0,"end":480}, ...]
        if (HookPoints.hasHook("chunk.split")) {
            val hookResult = HookPoints.callNullable<String>("chunk.split", mapOf("text" to text))
            if (hookResult != null) {
                try {
                    ...
                    for (i in 0 until arr.length()) {
                        ...
                        val chunkText = o.optString("text", "")
                        if (chunkText.isNotEmpty()) {
                            chunks += ChapterChunk(
                                chunkText,
                                o.optInt("start", o.optInt("startCharOffset", 0)),
                                o.optInt("end", o.optInt("endCharOffset", chunkText.length))
                            )
                        }
                    }
