        ...
        if (HookPoints.hasHook("dialogue.segment")) {
            ...
            if (hookResult != null) {
                try {
                    ...
                    val segments = mutableListOf<DraftAudiobookSegment>()
                    for (i in 0 until arr.length()) {
                        val o = arr.getJSONObject(i)
                        segments += DraftAudiobookSegment(
                            startCharOffset = o.optInt("start", o.optInt("startCharOffset")),
                            endCharOffset = o.optInt("end", o.optInt("endCharOffset")),
                            roleName = o.optString("role", o.optString("role", o.optString("roleName", "对白"))),
                            kind = run {
                                val k = o.optString("kind", "DIALOGUE").uppercase()
                                if (k.contains("NARR")) AudiobookSegmentKind.NARRATION else AudiobookSegmentKind.DIALOGUE
                            },
                            confidence = o.optDouble("confidence", 0.5).toFloat()
                        )
                    }
                    if (segments.isNotEmpty()) return segments.sortedBy(DraftAudiobookSegment::startCharOffset)
