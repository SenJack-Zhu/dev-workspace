...
object SentenceSegmenter {
    ...
    fun segment(body: String, maxChars: Int = DEFAULT_MAX_CHARS): List<SentenceSpan> {
        ...
        // ── Module hook: listen.sentence ──
        // JS modules return JSON: [{"start":0,"end":48}, ...]
        if (HookPoints.hasHook("listen.sentence")) {
            val hookResult = HookPoints.callNullable<String>("listen.sentence", mapOf(
                "body" to body, "maxChars" to maxChars
            ))
            if (hookResult != null) {
                try {
                    ...
                    for (i in 0 until arr.length()) {
                        ...
                        val s = o.optInt("start", 0)
                        val e = o.optInt("end", 0)
                        if (e > s) spans += SentenceSpan(s, e)
                    }
